...
try:
    ...
    THAI_SUPPORT_AVAILABLE = True
except ImportError:
    THAI_SUPPORT_AVAILABLE = False
    # Fallback if pythainlp or langdetect not available
...
def count_words(text):
    ...
    try:
        if THAI_SUPPORT_AVAILABLE:
            # Detect language and use appropriate tokenizer
            try:
                lang = detect(text)
                if lang == 'th':
                    # Use pythainlp for Thai text
                ...
                # Fallback to NLTK if detection fails
                tokenizer = nltk.tokenize.RegexpTokenizer(r"\w+")
                tokens = tokenizer.tokenize(text)
        else:
            # Fallback to NLTK if pythainlp not available
            tokenizer = nltk.tokenize.RegexpTokenizer(r"\w+")
            tokens = tokenizer.tokenize(text)
        num_words = len(tokens)
        return num_words
    except Exception:
        # Return 0 if tokenization fails
        return 0
