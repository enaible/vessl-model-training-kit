...
class ResponseLanguageChecker(Instruction):
    ...
    def check_following(self, value):
        ...
        try:
            if langdetect.detect(value) == self._language:
                return 1.0
            else:
                return 0.0
    ...
    def check_following(self, value):
        ...
        if self._comparison_relation == _COMPARISON_RELATION[0]:
            if num_sentences < self._num_sentences_threshold:
                ...
                error = abs(num_sentences - self._num_sentences_threshold) / self._num_sentences_threshold
...
class PlaceholderChecker(Instruction):
    ...
    def check_following(self, value):
        ...
        if num_placeholders >= self._num_placeholders:
            ...
            error = abs(num_placeholders - self._num_placeholders) / self._num_placeholders
            return max(0.0, 1.0 - error)
...
class BulletListChecker(Instruction):
    ...
    def check_following(self, value):
        ...
        error = abs(num_bullet_lists - self._num_bullets) / self._num_bullets
        return max(0.0, 1.0 - error)
...
class ConstrainedResponseChecker(Instruction):
    ...
    def check_following(self, value):
        ...
        Returns:
          1.0 if exact match, 0.8 if case-insensitive match, 0.5 if key words 
          present, 0.0 if no match.
        """
        value = value.strip()
        best_score = 0.0
        
        for constrained_response in self._constrained_responses:
            # Exact match (case-sensitive substring)
            if constrained_response in value:
                return 1.0
            
            # Good match: case-insensitive check
            if constrained_response.lower() in value.lower():
                best_score = max(best_score, 0.8)
                continue
            
            # Partial match: check if key words are present
            # Extract words from constrained response (exclude common words)
            words = [w.lower() for w in constrained_response.split() 
                     if w.lower() not in {'my', 'is', 'the', 'a', 'an'}]
            value_lower = value.lower()
            
            # If any significant word from constrained_response is in value
            if words and any(word in value_lower for word in words):
...
class ConstrainedStartChecker(Instruction):
    ...
    def check_following(self, value):
        ...
        Returns:
          1.0 if exact match, 0.8 if case-insensitive match, 0.5-0.7 if partial 
          word match, 0.0 if no match.
        """
        # First check exact match with regex
        ...
        # If not exact match, compare first n words
        value = value.strip()
        starter_words = self._starter.split()
        n = len(starter_words)
        
        # Extract first n words from response
        response_words = value.split()[:n]
        
        if not response_words:
            return 0.0
        
        # Case-insensitive exact match
        if ' '.join(response_words).lower() == self._starter.lower():
            return 0.8
        
        # Word-by-word matching
        matching_words = 0
        for i, starter_word in enumerate(starter_words):
            if i < len(response_words):
                # Exact word match (case-insensitive)
                if response_words[i].lower() == starter_word.lower():
                    matching_words += 1
                # Partial word match (one word contains the other)
                elif (starter_word.lower() in response_words[i].lower() or 
                      response_words[i].lower() in starter_word.lower()):
                    matching_words += 0.5
        
        if n > 0:
            match_ratio = matching_words / n
            # Scale to 0.5-0.7 range for partial matches
            return 0.5 + (match_ratio * 0.2)
